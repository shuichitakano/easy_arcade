...
出力に応じて発熱が増えますので、実用的には5V10A程度にとどめておくのが安全だと思います。
...
このスイッチを右側に設定するとCSyncをクランプし2Vまでに制限します（75Ω終端して1Vppになるはず…）。
...
また、ファームウェアのバグ等により、おかしな動作が起きることがよくあります。落ち着いて電源を抜き再起動するなどの対応を行なってください。
