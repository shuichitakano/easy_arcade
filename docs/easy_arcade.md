...
液晶画面は特に破損しやすいため、アーケード基板からの着脱の際などに力をかけないようにご注意ください。特に黒い部分が繊細なようです。
...
通常時の画面では、次の状態を確認できます。

- プレイヤーごとの連射周期（基本連射速度に対する分周比）
- フレームレート（V-Sync周波数）。DispFPSで表示を切り替えます
- 入力中のボタン、または連射が有効なボタン。BtnDispで表示を切り替えます
- 有効な機能：`A` はアナログモード、`R` はロータリーエンコーダ、`M` はマクロ
...
- VBtnCfg
  - Onにすると、BtnCfgでマクロ用の仮想論理ボタンG〜Jも割り当てます
  - デフォルトはOffです。
...
- Phase A-F
  - ボタンごとの連射位相を設定します
  - Front/Back: 連射速度で分周されたグローバル周期について、互いに逆の位相を選択します
  - Sync: ボタンを押した瞬間を周期の起点として、必ずOnから開始します
...
- ImpMcro
  - USBメモリのルートにある `.eamacro` ファイルを選択し、Aボタンでインポートします
  - 正常に読み込まれたプロファイルはFlashへ保存され、すぐに有効になります
  - USBメモリが接続されている時だけ表示されます

- SelMcro
  - Flashに保存されているマクロプロファイルを選択します
  - Disableを選ぶとマクロプロファイルを無効にします

- ImpCfg
  - USBメモリのルートにある `ea2_config.json` を本体へインポートします
  - STARTを押しながらAを押すと実行します
  - USBメモリが接続されている時だけ表示されます

- ExpCfg
  - 現在の本体設定をUSBメモリのルートに `ea2_config.json` としてエクスポートします
  - STARTを押しながらAを押すと実行します
  - USBメモリが接続されている時だけ表示されます
...
USBメモリのファイルシステムはFAT12、FAT16、FAT32およびexFATに対応しています。次の制限があります。

- 論理セクタサイズは512バイトのみ対応します
- 同時に使用できるUSBメモリとLUNは各1つです
- GPTには対応しません。MBRに複数のパーティションがある場合は、最初に見つかった対応ボリュームのみ使用します
- ロングファイル名は最大64文字です
...
マクロ機能では、VBtnCfgをOnにすると、通常のCOIN、START、方向、A〜Fに加えて、仮想論理ボタンG〜Jも割り当てられます。G〜Jに対応するJAMMA側出力はありませんが、マクロプロファイルのシーケンス起動やセレクタ操作などのトリガとして使用できます。
...
マクロプロファイルは、論理ボタンからJAMMA側出力へのマッピング、連射、入力シーケンス、マクロセット、ステートセレクタをまとめた `.eamacro` ファイルです。

`.eamacro` ファイルは、Webアプリの [EASY ARCADE Macro Studio](https://studio.easy-arcade.net/) で作成できます。作成したファイルをUSBメモリのルートへ保存し、ImpMcroから本体へインポートしてください。

USBメモリから読み込んだプロファイルは最大8件までFlashに保持されます。ファイル名は大文字・小文字を区別せずに同一判定し、同名ファイルを再度読み込むと上書きします。9件目を読み込んだ場合は、最後に使用してから最も時間が経過したプロファイルと入れ替えます。

プロファイルは1P・2P共通ですが、マクロの実行状態と現在のマクロセットはプレイヤーごとに独立しています。通常画面で `CMD + LEFT/RIGHT` を押すと、そのプレイヤーのマクロセットを前後へ切り替えます。端では先頭と末尾がつながり、切替操作に使用した左右入力はゲームへ出力されません。

マクロセットの選択はFlashへ保存されず、電源投入時は1P・2PともSet 0から開始します。マクロプロファイルを無効にしている場合、論理ボタンは従来どおりそのまま出力されます。


## 対応コントローラー
- USB-HID コントローラー(PS3, 4, 5 のコントローラーを含む)
- XBOX コントローラー
- Switch Pro系USBコントローラー（同じUSB ID・プロトコルを使用する互換機を含む）
- USB-MIDI
...
[v1.3.1](https://github.com/shuichitakano/easy_arcade/releases/download/v1.3.1/firmware_v1_3_1.zip)

ファームウェアをアップデートすると全ての設定がリセットされることがあります。
あらかじめご了承ください。

**⚠️ V1.0 のUSB micro コネクタは強固に固定されていません。挿抜時の破損にご注意ください。**

## ケース
壊れやすい部分がありますので、なるべく何かしらのケースに入れて運用することをお勧めします。
別途販売もしていますが、3Dプリントできるデータも公開しています。
[ダウンロード](model/case.zip)
...
## V1.2のおまけ機能について
V1.2には5Vの電圧調整を行うことが可能となるポテンショメーターを実装できるランドが設けられています。

簡単な改造を行うことで電圧範囲を4V-6Vで調整することができますが、それなりの半田付け技術が必要になりますので、表面実装部品の実装に慣れていない方は作業をお控えください。また、改造済みEASY ARCADEの修理はお断りさせて頂くことがあります。

### 使用する部品
取り付けられるポテンショメーターはPVZ3A222C01です。
![ポテンショメーター](ea2_v1_2_trimmer.jpg)

### 改造
写真を参考にジャンパの変更とポテンショメーターの実装を行ってください。
1. RV5 を実装する
  - 1ピンだけランドにハンダを盛って部品を実装、その後に残りのピンを半田付けとする
  と簡単だと思います
2. JP3 のハンダを除去する
3. JP3 の下側２つをハンダでブリッジ

![RV5を実装](ea2_v1_2_rv5.jpg)

### 電圧調整
時計回り方向に回すと電圧が上昇します。
最大6V付近まで電圧調整可能ではありますが、5.5V以上に設定すると、アーケード基板やUSB機器にダメージを与える危険性があります。
